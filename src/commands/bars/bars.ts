    ...
    $: {
      fn: async ({ $ }, subCalls) => {
        const longestCall = Math.max(
          ...Object.keys(subCalls).map((sc) => sc.length)
        )
        const ownLength = `bars $`.length
        if (ownLength === longestCall) {
          const [phaseName] = $
          return Object.fromEntries(
            Object.entries(mem().notesByBar)
              .filter(([k]) => k.startsWith(`${phaseName}:`))
              .map(([k, v]: [k: string, v: NoteByBar[]]) => {
                const allChords = v.reduce(
                  (accum: string[], curr: NoteByBar) => {
                    const parsed = Object.fromEntries(parseNoteTags(curr.tags))
                    const [chordName] = parsed.chord
                    if (typeof chordName !== 'string')
                      throw new Error(`Invalid chord identifier ${chordName}`)
...
`,
          ...
          fn: async ({ $: dollar, positionalNonCommands }) => {
            ...
            if (bars.length === 0)
              throw new Error(`Phase ${phaseName} has no bars`)
            ...
            rawObjects.forEach((str: string, objIdx: number) => {
              ...
              if (isChordCsvArg(str)) {
                const [notes, tags] = parseChordCsvArg(
                  str,
                  `${scaleTonic} ${scaleName}`
                )
                if (notes.length === 0) {
                  throw new Error(
                    `Error; ${str} could not be parsed to anything with notes`
                  )
                }

                const fn = makeFulfilledBarNote(barTag, [
                  ...commonTags,
                  ...tags,
                  ...timingTags,
                  groupIdTag,
        ...
        add: {
          help: {
            description:
              'Add chord bars ore note-group bars to a phrase; like `fill`; but it creates a new phase and tacks it onto the end of the named one',
            examples: {
              'Em,3 Am,3 [] C3,E3,G#3': `
...
`,
          ...
          fn: async ({ $: dollar, positionalNonCommands }) => {
            ...
            const bars = getAllPhaseBars(phaseName)
            const barIdxs = bars.map((barName) =>
              parseInt(barName.split(':')[1])
            )
        ...
        remove: {
          ...
          fn: async ({ $: dollar }) => {
            ...
            mem().notesByBar = {
              ...Object.fromEntries(
                Object.entries(mem().notesByBar).filter(
                  ([k, v]: [k: string, v: NoteByBar[]]) => {
                    return k !== deleteable
                  }
                )
              ),
...
`,
          ...
          fn: async ({ $: dollar, pack = [], stuff = [] }) => {
            ...
            if (pack.length > 0 && stuff.length > 0) {
              throw new Error(
                'Either --pack or --stuff is required, but not both'
              )
            }
            if (pack.length === 0 && stuff.length === 0) {
              throw new Error('Either --pack or --stuff is required')
            }
            const packOrStuff: 'pack' | 'stuff' =
              pack.length > 0 ? 'pack' : 'stuff'
            const packk = pack.length > 0 ? pack : stuff

            // strip existing bar tag
            bars.forEach((barTag) => {
              detachedBars.push(notesByBar[barTag])
              notesByBar[barTag] = []
            })
            ...
            if (
              ...
              abbreviations.length !== packk.length
            ) {
              throw new Error(
                `Plan for packing or stuffing must be all fractions or all numeric`
              )
            }

            const packPlanType: 'NUMERIC' | 'FRACTIONAL' =
              abbreviations.length === 0 ? 'NUMERIC' : 'FRACTIONAL'
            ...
            switch (caseName) {
              ...
              case 'fractional-pack':
                throw new Error(
                  '--pack cannot be used with a fractional plan; instead whole-number eg "--pack 0 2 4 1" to resequence chords into the whole song'
                )
...
function numericPack(
  bars: string[],
  packk: string[],
  detachedBars: NoteByBar[][],
  barSizeMod: number
) {
  bars.forEach((barTag) => {
    ...
    while (nextGroup.length < packPlan) {
      nextGroup.push(detachedBars.shift())
    }
    const interim = Math.trunc(
      (tickCounts[BAR] * (barSizeMod || 1)) / nextGroup.length
    )
...
function numericStuff(
  ...
  throw new Error(
    '--stuff cannot be used with a numeric plan; (what would that even look like?) instead use fractional eg "--stuff 4th,8th half 4th,16th"'
  )
}

function fractionalStuff(
  bars: string[],
  packk: string[],
  detachedBars: NoteByBar[][]
) {
  ...
  const absolutizedPackTranslation = packk.reduce(
    (accum: number[], currCsv: string) => {
  ...
  const newNotesByBar = Object.fromEntries(
  ...
  while (barRanges.length) {
    ...
    absolutizedPackTranslation.forEach((numRaw: number, groupIdx: number) => {
      ...
      const found = barRanges.find(([start, end]) => {
      ...
      if (found) {
        ...
        const adjustedGroupIdx =
          packIterations * absolutizedPackTranslation.length + groupIdx
        ...
        newNotesByBar[bars[foundIdx + offset]].push(
          ...notes.map((n) => ({
            ...filterBarDelayTag(n),
            tags: [...n.tags, `barDelay=${num - found[0]}`],
          }))
        )
      }
    })
