...
export const chord: Module = {
  help: {
    description:
      'This is the chord module! Utilities and arrangement attentive to chords.',
  },
  fn: async () => {
    return null
  },
  submodules: {
        ...
        return {
          formatted: formatted.filter(
            ({ scaleTonic, scaleType }) =>
              scaleTonic === userLetter && scaleType === userScale
          ),
        }
      },
    },
    fromRoman: {
      fn: async (args) => {
        const [userLetter = '', userScale = '', romanName = null] =
    ...
    progressions: {
      fn: async (args) => {
        const [userLetter = '', userScale = ''] = args.positionalNonCommands
        const allTriadsRaw = await fakeCli(
          `chord triads ${userLetter} ${userScale}`,
          'cli'
        )
        const allTriads = z
          .array(
        ...
        const formatted = allTriads.map(({ scaleTonic, triads, scaleType }) => {
          return Progression.toRomanNumerals(scaleTonic, triads).map(
            (romanNum, idx) => {
      ...
      submodules: {
        ...
        song: {
          fn: async ({ positionalNonCommands }) => {
            ...
            } = (await fakeCli(`chord graph create ${userLetter} ${userScale}`))
              .formatted
            ...
            const chordsWithNotes = chords.map((someChord: string) => {
              ...
              if (isChordFn(someChord)) {
                const tmp = fnChordNameWithNotes(
                  someChord,
                  userLetter,
                  userScale
                )
                chord = {
                  ...tmp,
                  notes: tmp.notes.map((n) => {
                    return `${n}${oct}`.toLowerCase()
                  }),
                }
              } else {
            ...
            const noteStr = chordsWithNotes
              .map(({ notes }) =>
                notes.map(Note.simplify).join(',').toLowerCase()
              )
              .join(' ')
              ...
              submodules: {
                $: {
                  fn: async ({ $: dollar, positionalNonCommands }) => {
                    ...
                    const [chordName] = parseNoteTags(
                      latestChordNote.tags
                    ).find(([nm]) => nm === 'chord')[1]
                    ...
                    if (typeof chordName !== 'string') {
                      throw new Error(
                        `could not get chord name; instead ${chordName}`
                      )
                    }
                    ...
                    if (!graph) {
                      await fakeCli(
                        `chord graph create ${scaleName[0]} ${scaleName[1]}`
                      )
                    ...
                    if (!graph[chordName]) {
                      throw new Error(
                        `could not obtain ${chordName} in graph for ${scaleName}`
                      )
                    }
      ...
      submodules: {
                    ...
                    arrange: {
                      fn: async ({
                        ...
                        const notes1 = await fakeCli(
                          ...
                          if (Array.isArray(notesResult)) {
                              ...
                              .array(
                                z.object({
                                  note: z
                                    ...
                                    .refine(
                                      (str) =>
                                        isNoteNameWithOctave(str) ?? false
                                    ),
                            ...
                            if (notesWithTags.success === false) {
                              ...
                              throw new Error(
                                `Incorrectly formatted or empty notes from cli boundary: ${strjson(notesResult)}`
                              )
                            }
                        ...
                        if (parsed.success === false) {
                          ...
                          throw new Error(
                            `Incorrectly formatted or empty notes:${strjson(notes1)}`
                          )
                        }
                        ...
                        const notes = parsed.data.filter((n) => {
                          ...
                          if (isMatch) {
                            const chordSize = parsedTags.find(
                              ([nm, dat]) => nm === 'chordSize'
                            )
                            if (
                              chordSize &&
                        ...
                        const prepped = prepDelayMatrix(
                          patterns as ParsedCli['positionalNonCommands']
                        )
        ...
        aliases: {
          fn: async (args, moduleCalls) => {
            ...
            const aliases = [notes, ...notesPermuted]
              .map((notes1) => Chord.detect(notes1))
              .flat()
