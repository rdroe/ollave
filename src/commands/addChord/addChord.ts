...
export default {
  ...
  fn: async ({
    ...
    if (!phaseName || !barIndex || !peprnIsNum(barIndex)) {
      throw new Error(
        'Phase should match phaseName:barIndex; instead got ' + barName
      )
    }
    ...
    if (!finalArp) {
      throw new Error(
        'Arp must be a string array, number array, or the number 1, 2, or 3'
      )
    }

    if (
      !['string', 'undefined'].includes(typeof scaleTonic) ||
      !['string', 'undefined'].includes(typeof scaleName)
    ) {
      throw new Error('Scale tonic and scale name must be strings')
    }
    const retVar = addChord(
      chordName,
      phaseName,
      parseInt(barIndex),
      finalArp,
      tags,
      z.string().or(z.undefined()).parse(scaleTonic),
      z.string().or(z.undefined()).parse(scaleName),
      z.boolean().or(z.undefined()).parse(addSlider) ?? false
    )
