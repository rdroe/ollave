...
const cliDelaysToTags = (delay?: string[]): string[] => {
  ...
  delay.forEach((str) => {
    if (isAbbreviation(str)) {
      ...
      tots[fractConst] =
        tots[fractConst] !== undefined ? tots[fractConst] + 1 : 1
    } else {
      console.error(`${str} is not an abbreviation for a fractional note`)
    }
  })

  const tags = Object.entries(tots).map(
    ([tagName, count]: [tn: keyof typeof tickCounts, count: number]) => {
                ...
                add: {
                  ...
                  fn: async ({ $: dollar, positionalNonCommands, delay }) => {
                    ...
                    if (!isChordCsvArg(chordName)) {
                      throw new Error(
                        `Chord and octave csv required; instead  "${chordName}"`
                      )
                    }
                    ...
                    const addNote = makeFulfilledBarNote(barTag, [
                      groupIdTag,
                      layerTag,
                      placementTag,
                      ...chordTags,
