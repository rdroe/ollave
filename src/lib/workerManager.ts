  ...
  private initializeWorker() {
    try {
      ...
      if (workerUrl) {
        ...
        // Fallback: try to load the compiled worker file
        this.worker = new Worker('./mapSongToTicksWorker.js')
      }
    ...
    if (message.type === 'MAP_SONG_TO_MIDI_TICKS_RESULT') {
      ...
      if (firstPending) {
        try {
          ...
          const deserializedResult = deserializeMidiMappingResult(message.data)
