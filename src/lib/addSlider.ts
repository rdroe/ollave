...
const syncSliderValue = (noteId: string) => {
  ...
  const newVal = Object.values(mem().notesByBar)
    .flat()
    .find((note) => note.tags.includes(`noteId=${noteId}`))?.tagsObj.barDelay[0]
...
export function addSlider(barName: string, noteId: string) {
  ...
  const noteData = mem().notesByBar[barName].find((note) =>
    note.tags.includes(`noteId=${noteId}`)
  )
  const noteDelay = noteData?.tags
    .find((tag: string) => tag.startsWith('barDelay='))
    ?.split('=')[1]
  if (typeof noteDelay !== 'string' || !peprnIsNum(noteDelay)) {
    console.error(
      'barDelay datum should be a number; insteaed got ' + noteDelay
    )
  ...
  slider.oninput = () => {
    ...
    const quantizedBarDelay = isAbbreviation(abbrev)
      ? quantizeValueToAbbreviation(parseFloat(slider.value), abbrev)
...
export function updateBarDelayFromSlder(
  noteData: NoteByBar,
  newBarDelay: number,
...
export function updateTagsObj(
  id: string,
  tagsObj: z.infer<typeof tagsObjSchema>,
  skipSliderSync: boolean = true
) {
  const noteData = Object.values(mem().notesByBar)
    .flat()
    .find((note) => note.tags.includes(`noteId=${id}`))
  if (!noteData) {
