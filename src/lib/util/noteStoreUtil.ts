...
export const createNoteStoreById = (noteId: string) => {
  const note = Object.values(mem().notesByBar)
    .flat()
    .find((note) => note.tags.includes(`noteId=${noteId}`))
  if (!note) {
  ...
  const updateTagsObj = (newTagsObj: { [key: string]: any }) => {
    ...
    const updatedTags = Object.entries(newTagsObj).map(
      ([key, value]) => `${key}=${value.join(',')}`
    )
    currentNote.tags = tagsDeleteMatching1(
      (tagEntry) => !Object.keys(newTagsObj).includes(tagEntry[0]),
      parseNoteTags(currentNote.tags)
    )
      .map(([k, v]) => `${k}=${v.join(',')}`)
      .concat(updatedTags)
