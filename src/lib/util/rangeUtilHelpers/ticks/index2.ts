...
type Fns = {
  [rangeId: string]: {
    [ticksType: string]: (inputRange: [start: number, end: number]) => TicksArray<number>
  }
}
const fns: Fns = {}
type EventDetails = {
...
// access conversionStore and get the viewable range data. 
function getTicksLoadingStartHandler(rangeId: string, rangeName: "viewableRange" | "nextLeftRange" | "nextRightRange") { 
  return function () {
    ...
    const ticks = fns[rangeId].createDefaultTicks(range)
...
export const registerTicks = (rangeId: string, createDefaultTicks: (inputRange: [start: number, end: number]) => TicksArray<number>) => {
  fns[rangeId] = {
