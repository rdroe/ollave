...
function getErrorLineNumber(error: Error): string | null {
  ...
  // Look for the first stack frame that references rangeUtilTests.ts
  for (const line of stackLines) {
    const match = line.match(/rangeUtilTests\.ts:(\d+):(\d+)/)
    if (match) {
  ...
  registerRange(
    ...
    if (!testName || testName === 'updateRange' || testName === '') {
      try {
        ...
        results.updateRangeInputInnerNumeric = createTestResult(
          false,
          'updateRangeInputInputInner for numeric range test failed',
          error
        )
      }
    }
